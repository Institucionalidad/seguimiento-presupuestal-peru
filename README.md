# Seguimiento de la Ejecución Presupuestal — Perú 2026

Tablero de seguimiento de la ejecución del gasto público peruano 2026, con datos abiertos del MEF (Consulta Amigable — Gasto Diario). Publicado en:

**https://seguimiento-presupuestal.vidartegarcia.com/**

## Qué hay en este repositorio

- `index.html` — el tablero, un archivo HTML/CSS/JS autocontenido (los datos ya están incrustados adentro como JSON; no depende de ningún backend). Esto es lo único que GitHub Pages sirve.
...
- `inject_data.py` — inyecta esos cubos dentro de la plantilla y genera `index.html`.
...
3. Correr `4. Tablero/inject_data.py` — regenera `4. Tablero/index.html` con los datos nuevos.
4. Subir el `index.html` nuevo a este repositorio (reemplazando el actual, mismo nombre) — por la interfaz web de GitHub ("Add file → Upload files") o con `git`.
